...
cmdParser.add_argument('-s',
					'--srcPort',
					dest='sourPort',
					help='Source port of the host to send the command to.',
					required=True)
...
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
def packetCraft(destIP, sourPort, destPort, data):
    ...
    craftedPacket = IP(dst=destIP, ttl=188)/UDP(sport=int(sourPort), dport=int(destPort))/Raw(load=data)
...
if __name__ == "__main__":
	...
	while True:
		...
		#After the command is inputted, sniff for the output result.
			...
			#sniffFilter = "tcp and dst port {0}".format(dstPort)
			sniffFilter = "udp"
			sniff(filter=sniffFilter, prn=getOutput, stop_filter=packetCheck)
			sendFlag = True
